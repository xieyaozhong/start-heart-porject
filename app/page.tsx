  ...
  return (
    <main className="public-site">
      <header className="site-header">
        ...
        <nav><a href="#solar-system">Solar System</a><a href="#observatory">Candidate Systems</a><a href="#discoveries">Discoveries</a><a href="#registry">Private Registry</a><a href="/resources">Institutions</a><button onClick={() => setRegistryOpen(true)}>Holder Access</button></nav>
        <a className="admin-link" href="/admin">CONTROL ROOM ↗</a>
      </header>
      ...
      <section className="registry-section" id="registry">
        <div className="registry-copy"><p className="eyebrow">PRIVATE CELESTIAL REGISTRY</p><h2>Place a name<br /><em>into lasting orbit.</em></h2><p>Once confirmed, each holder receives a unique stellar-system registry number, a personalised naming certificate and a dedicated animation generated from live orbital data.</p><button onClick={() => setRegistryOpen(true)}>HAVE A REGISTRY NUMBER? OPEN YOUR SYSTEM →</button></div>
      ...
      <footer><div className="brand"><span className="brand-sigil small">N</span><span><b>NOCTUA</b><small>CELESTIAL RESEARCH LAB</small></span></div><p>Model outputs are provided for education, research hypotheses and private memorial use. Private registry names are not official IAU designations.</p><div className="footer-links"><a href="/resources">Astronomy institutions</a><a href="/admin">Control room</a></div></footer>

      {orderPlan && <div className="modal-shell" onMouseDown={() => !orderBusy && setOrderPlan(null)}><section className="order-modal" onMouseDown={(event) => event.stopPropagation()}><button className="modal-close" disabled={orderBusy} onClick={() => setOrderPlan(null)}>×</button>{orderDone ? <div className="order-success"><span>✓</span><p>REGISTRY ORDER CREATED</p><h2>{orderDone}</h2><small>Your unique registry number and animation access will be issued after payment confirmation.</small><button onClick={() => setOrderPlan(null)}>DONE</button></div> : <form onSubmit={submitOrder}><p className="eyebrow">MEMORIAL ORDER</p><h2>{orderPlan.name} Plan</h2><div className="order-target"><span>REGISTRY TARGET</span><b>{system.designation} · {planet.code.split(" ").at(-1)}</b></div><label>Memorial name<input name="desiredName" required maxLength={40} placeholder="e.g. Asteria" /></label><label>Holder name<input name="ownerName" required maxLength={60} /></label><label>Email address<input name="email" required type="email" /></label><label>Dedication<textarea name="dedication" maxLength={240} rows={3} placeholder="A sentence to preserve on the certificate" /></label><div className="secure-payment"><div><span>SECURE CHECKOUT</span><b>ECPay</b></div><p>Credit card · ATM · convenience-store code · mobile payment</p><small>Payment details are entered on ECPay’s encrypted page. NOCTUA never receives or stores your card number.</small></div><button className="primary-action" type="submit" disabled={orderBusy}>{orderBusy ? "PREPARING SECURE CHECKOUT…" : `PROCEED TO PAYMENT · NT$ ${orderPlan.priceTwd.toLocaleString()} →`}</button><p className="payment-terms">By continuing, you acknowledge that private memorial names are not official IAU designations.</p></form>}</section></div>}

      {registryOpen && <div className="modal-shell" onMouseDown={() => setRegistryOpen(false)}><section className={registry ? "owner-modal active" : "owner-modal"} onMouseDown={(event) => event.stopPropagation()}><button className="modal-close" onClick={() => { setRegistryOpen(false); setRegistry(null); }}>×</button>{!registry ? <form onSubmit={lookupRegistry}><p className="eyebrow">HOLDER ACCESS</p><h2>Open your private system</h2><p>Enter the NOCTUA registry number issued after payment confirmation.</p><div className="demo-owner-account"><div><span>DEMO HOLDER ACCOUNT</span><b>Asteria Noctua</b><small>Example holder · Starlight archive</small></div><code>{DEMO_OWNER_REGISTRY_CODE}</code><button type="button" onClick={openDemoRegistry}>OPEN DEMO SYSTEM →</button></div><div className="owner-account-divider"><span>OR USE A HOLDER REGISTRY NUMBER</span></div><label>Registry number<input value={registryCode} onChange={(event) => setRegistryCode(event.target.value.toUpperCase())} required placeholder="NOR-XXXXXXXX" /></label>{registryError && <span className="form-error">{registryError}</span>}<button className="primary-action" type="submit">VERIFY & OPEN →</button></form> : <div className="owner-experience"><div className="owner-sky"><OrbitCanvas system={registry.system} selectedId={registry.system.planets[0].id} onSelect={() => undefined} mode="animation" ownerLabel={registry.order.desiredName} /></div><div className="owner-certificate"><p>NOCTUA PRIVATE REGISTRY</p><h2>{registry.order.desiredName}</h2><span>MEMORIAL REGISTRY HOLDER · {registry.order.ownerName}</span><div><small>UNIQUE STELLAR-SYSTEM REGISTRY</small><b>{registry.order.registryCode}</b><small>SCIENTIFIC DESIGNATION</small><b>{registry.system.designation}</b></div>{registry.order.dedication && <blockquote>“{registry.order.dedication}”</blockquote>}<button className="owner-3d-button" onClick={() => setExplorerTarget({ system: registry.system, planetId: registry.system.planets[0].id, ownerLabel: registry.order.desiredName, registryCode: registry.order.registryCode })}>OPEN PRIVATE SYSTEM IN 3D ↗</button><button onClick={() => { setRegistry(null); setRegistryCode(""); }}>BACK TO LOOKUP</button></div></div>}</section></div>}

      {explorerTarget && <Suspense fallback={<div className="explorer-loading"><span>N</span><b>LOADING THE 3D UNIVERSE…</b></div>}><CelestialExplorer3D system={explorerTarget.system} initialPlanetId={explorerTarget.planetId} ownerLabel={explorerTarget.ownerLabel} registryCode={explorerTarget.registryCode} onClose={() => setExplorerTarget(null)} /></Suspense>}
    </main>
  );
}
