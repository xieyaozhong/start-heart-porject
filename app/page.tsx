...
type NamingPackage = { id: string; name: string; priceUsd: number; priceTwd: number; description: string; features: string[] };
type Registry = {
...
const fallbackPackages: NamingPackage[] = [
  { id: "PKG-EXPLORER", name: "Explorer", priceUsd: 200, priceTwd: 6000, description: "A private memorial registry for one candidate planet", features: ["Digital naming certificate", "Live orbital animation", "Unique planetary designation"] },
  { id: "PKG-OBSERVER", name: "Observer", priceUsd: 300, priceTwd: 9000, description: "A star and its complete candidate planetary system", features: ["Unique stellar-system registry", "4K system animation", "Annual position update"] },
  { id: "PKG-ARCHIVIST", name: "Archivist", priceUsd: 500, priceTwd: 15000, description: "A complete archival record with bespoke presentation", features: ["Print-ready archival certificate", "Custom dedication and thank-you letter", "Research dossier and high-resolution celestial artwork"] },
];
...
function OrbitCanvas({ system, selectedId, onSelect, mode, ownerLabel, speed = 1, paused = false }: { system: StarSystem; selectedId: string; onSelect: (id: string) => void; mode: "live" | "animation"; ownerLabel?: string; speed?: number; paused?: boolean }) {
  ...
  useEffect(() => {
    ...
    const draw = (time: number) => {
      ...
      if (tripleSystem) {
        ctx.save(); ctx.setLineDash([2,4]); ctx.strokeStyle = "rgba(117,193,210,.25)"; ctx.lineWidth = .8; ctx.beginPath();
        for (let step = 0; step <= 96; step += 1) { const phase = step / 96 * Math.PI * 2; const x = cx + Math.sin(phase) * starRadius * 2.45; const y = cy + Math.sin(phase) * Math.cos(phase) * starRadius * 1.45; if (step === 0) ctx.moveTo(x,y); else ctx.lineTo(x,y); }
      ...
      const drawAdditionalStar = (x: number, y: number, radius: number, cool = false) => {
        const pairCorona = ctx.createRadialGradient(x, y, 0, x, y, radius * 4.4);
        pairCorona.addColorStop(0, cool ? "rgba(231,247,255,1)" : "rgba(255,239,196,1)"); pairCorona.addColorStop(.25, cool ? "rgba(111,181,255,.72)" : "rgba(255,151,55,.7)"); pairCorona.addColorStop(1, cool ? "rgba(64,112,255,0)" : "rgba(255,91,25,0)");
        ctx.fillStyle = pairCorona; ctx.beginPath(); ctx.arc(x, y, radius * 4.4, 0, Math.PI * 2); ctx.fill();
        const pairSurface = ctx.createRadialGradient(x - radius * .3, y - radius * .3, 1, x, y, radius);
      ...
      <div className="universe-distance-map" role="img" aria-label={`Schematic Sun-centred distance map showing ${systems.length} model candidate systems out to ${maxDistance} parsecs`}>
        ...
        {systems.map((item) => {
          ...
          return <div className={item.id === selected.id ? "universe-system-point active" : "universe-system-point"} style={style} key={item.id}>
            ...
            <button type="button" onClick={() => setSelectedMapId(item.id)} aria-pressed={item.id === selected.id} aria-label={`Select ${item.id}, ${item.designation}, ${item.distancePc.toFixed(1)} parsecs from the Solar System`}><i className={stellarMark} /><b>{item.id}</b><small>{item.designation} · {item.distancePc.toFixed(1)} pc</small></button>
          </div>;
        })}
      </div>
  ...
  return (
    <main className="public-site">
      ...
      <section className="registry-section" id="registry">
        ...
        <div className="package-list">{packages.map((item, index) => <article key={item.id} className={index === 1 ? "package-card featured" : "package-card"}><div><span>{item.name}</span>{index === 1 && <i>RECOMMENDED</i>}</div><h3>US$ {item.priceUsd.toLocaleString()}</h3><p>{item.description}</p><ul>{item.features.map((feature) => <li key={feature}>＋ {feature}</li>)}</ul><button onClick={() => { setOrderPlan(item); setOrderDone(null); setOrderBusy(false); }}>SELECT THIS PLAN</button></article>)}</div>
      </section>
