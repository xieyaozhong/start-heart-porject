...
function SolarSystemCanvas({ selectedId, onSelect, mode, speed, paused }: { selectedId: string; onSelect: (id: string) => void; mode: "live" | "animation"; speed: number; paused: boolean }) {
  ...
  return <canvas ref={canvasRef} className="solar-canvas" aria-label="太陽、八大行星與九顆知名衛星的即時近似位置，以及不顯示軌道線的哈雷彗星；點擊星體可查看資訊" />;
}
  ...
  return (
    <main>
      <header className="site-header">
        ...
        <nav><a href="#solar-system">太陽系</a><a href="#observatory">候選星系</a><a href="#discoveries">最新發布</a><a href="#registry">紀念命名</a><a href="/resources">全球天文機構</a><button onClick={() => setRegistryOpen(true)}>持有者入口</button></nav>
      ...
      <section className="observatory" id="observatory">
        ...
        <div className="observatory-grid">
          ...
          <aside className="planet-inspector">
            ...
            <button className="open-3d-explorer" onClick={() => setExplorerTarget({ system, planetId: planet.id })}><span>沉浸式天體探索</span><b>開啟 3D VIEW ↗</b></button>
          </aside>
        </div>
      </section>
      ...
      {explorerTarget && <Suspense fallback={<div className="explorer-loading"><span>N</span><b>正在載入 3D 宇宙…</b></div>}><CelestialExplorer3D system={explorerTarget.system} initialPlanetId={explorerTarget.planetId} ownerLabel={explorerTarget.ownerLabel} registryCode={explorerTarget.registryCode} onClose={() => setExplorerTarget(null)} /></Suspense>}
    </main>
  );
}
