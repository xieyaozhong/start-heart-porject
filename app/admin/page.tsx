...
type OrderRow = { id: string; desiredName: string; ownerName: string | null; packageName: string; amountTwd: number; status: string; registryCode: string | null; createdAt: string };
type RunRow = { id: string; source: string; status: string; generatedSystemId: string | null; startedAt: string };
type Dashboard = { systems: SystemRow[]; packages: PackageRow[]; orders: OrderRow[]; runs: RunRow[]; settings: Record<string, string> };
...
export default function AdminPage() {
  ...
  return <main className="admin-shell">
    ...
    <section className="admin-main"><header><div><p>INFERENCE OPERATIONS</p><h1>{tab === "overview" ? "系統總覽" : tab === "systems" ? "星體推算與發布" : tab === "packages" ? "命名方案管理" : "訂單與專屬登錄"}</h1></div><span><i /> 排程服務在線</span></header>{error && <div className="admin-error">{error}</div>}
      ...
      {tab === "orders" && <div className="admin-card wide"><div className="card-head"><div><span>命名登錄訂單</span><small>{dashboard.orders.length} ORDERS</small></div></div><div className="admin-table orders-table"><div className="table-row table-head"><span>訂單</span><span>紀念名稱／持有者</span><span>方案</span><span>金額</span><span>狀態／編號</span><span>操作</span></div>{dashboard.orders.map((item) => <div className="table-row" key={item.id}><span><b>{item.id}</b><small>{new Date(item.createdAt).toLocaleDateString("zh-TW")}</small></span><span><b>{item.desiredName}</b><small>{item.ownerName}</small></span><span>{item.packageName}</span><span>NT$ {item.amountTwd.toLocaleString()}</span><span><i className={`status ${item.status}`} />{item.registryCode ?? "等待確認"}</span><span>{item.status === "confirmed" ? <b className="confirmed">已核發</b> : <button onClick={() => action({ action: "approve_order", id: item.id }, `order-${item.id}`)}>確認並核發</button>}</span></div>)}</div></div>}
    </section>
  </main>;
}
